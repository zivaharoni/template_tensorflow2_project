...
import logging
from backend.loss_metric_utils import nan_mask, identity, nan_mask_5_7, argmax
...
class ClassificationMetrics(tf.keras.metrics.Metric):
    ...
    def __init__(self, writer, name='', **kwargs):
        ...
        self.writer = writer
        self.metric_pool = [tf.keras.metrics.SparseCategoricalCrossentropy(name='CE'.format(name)),
                            Accuracy(name='accuracy'.format(name)),
                            Accuracy(name='accuracy_5_7'.format(name)) ]
